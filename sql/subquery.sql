...
SELECT m.*
FROM movies m
JOIN top5_combos tc ON m.star = tc.star 
                   AND m.director = tc.director;
...
-- Answer:
WITH customer_food_frequency AS (
    SELECT 
        u.user_id,
...
SELECT 
    name, 
    votes,
    ROUND((votes * 100.0 / (SELECT SUM(votes) FROM movies)), 2) AS vote_percentage
FROM movies
...
SELECT 
    ...
    (SELECT ROUND(AVG(score), 2) FROM movies) AS overall_avg_score
FROM movies 
GROUP BY genre
HAVING AVG(score) > (
    SELECT AVG(score) 
...
-- Answer:
INSERT INTO loyal_customers (user_id, name, order_count)
SELECT 
    o.user_id,
    u.name,
